...
class TestMainSessionPersistence:
    ...
    @patch('src.session.session_manager.SessionManager.restore_session')
    @patch('src.session.session_manager.SessionManager.save_session')
    ...
    def test_save_session_on_exit(self, mock_save, mock_input, temp_state_file):
        ...
        with patch('src.application.main.SessionManager', return_value=SessionManager(state_manager)):
            with patch.object(sys, 'argv', ['main.py']):
                main()
